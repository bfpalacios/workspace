...
import pe.gob.produce.produccion.model.ArchivoModel;
...
public class DatosAlumnosMBean {
	...
	public String handleFileUpload(FileUploadEvent event) throws Exception {
        ...
        try {        	 
        	...
        	FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Ocurrió un error al leer el archivo" + e, null));
        	e.printStackTrace();
        }
        
        switch(MODO_USUARIO){
        	case 1: pagina = "/paginas/ModuloObservados/admin/cargar/cargarDatosAlumnosObs.xhtml"; break;
        	case 2: pagina = "/paginas/ModuloObservados/ocaa/cargar/cargarDatosAlumnosObs.xhtml"; break;
        }        
        return pagina;
    }
	...
	public void setExcel(UploadedFile excelCargado, FileUploadEvent event){
		if(excelCargado != null) {
        	...
        	FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "El archivo no se ha cargado correctamente", null));
        }
	}
	...
	public DatosAlumnoExcelModel  convertirAModelAlumno(XSSFRow fila_alumno){
		...
		try{
			datos.setCod_alumno(fila_alumno.getCell(COD_ALUMNO)!=null?validarDatoEntero(fila_alumno.getCell(COD_ALUMNO)).toString():"0");
			...
			datos.setDes_curso(fila_alumno.getCell(NOM_CURSO)!=null?validarDatoCadena(fila_alumno.getCell(NOM_CURSO)).toString():"");
			datos.setCreditos(fila_alumno.getCell(NUM_CRED)!=null?validarDatoEntero(fila_alumno.getCell(NUM_CRED)).toString():"0");
			datos.setRepitencias(fila_alumno.getCell(REPITENCIAS)!=null?validarDatoEntero(fila_alumno.getCell(REPITENCIAS)).toString():"0");
			datos.setCod_docente(fila_alumno.getCell(COD_DOCENTE)!=null?fila_alumno.getCell(COD_DOCENTE).toString():"");
			datos.setNom_docente(fila_alumno.getCell(NOM_DOCENTE)!=null?validarDatoCadena(fila_alumno.getCell(NOM_DOCENTE)).toString():"");
			datos.setCod_frecuencia(fila_alumno.getCell(FRECUENCIA)!=null?validarDatoEntero(fila_alumno.getCell(FRECUENCIA)).toString():"0");	
//			datos.setNom_frecuencia(comunServices.obtenerDatosFrecuencia(Integer.parseInt(fila_alumno.getCell(FRECUENCIA)!=null?
//	                                                                     validarDatoEntero(fila_alumno.getCell(FRECUENCIA)).toString():"0")).getValorCampo());
			datos.setDia(fila_alumno.getCell(DIA)!=null?fila_alumno.getCell(DIA).toString():""); 
			datos.setHora_inicio(fila_alumno.getCell(HORA_INICIO)!=null?fila_alumno.getCell(HORA_INICIO).toString():""); 
	...
	private DatosAlumnoExcelModel validarRegistroTutoria(DatosAlumnoExcelModel datos){
		if (datos.getCod_alumno() == "" || datos.getCod_alumno().length() > 8 || datos.getAp_paterno() == "" || datos.getAp_materno() == "" || 
			datos.getNombres() == "" || datos.getCod_plan() == "0" || datos.getCod_curso() == "" || datos.getRepitencias() == "0" || 
			datos.getCod_docente() == "" || datos.getCod_frecuencia() == "0" || datos.getDia() == "" || datos.getHora_inicio() == "" || 
	...
	public void mostrar() {
	 ...
	 private void mostrarMensaje(int opcionMensaje){
		 ...
		 switch(opcionMensaje){
		 	case 1: message = new FacesMessage(FacesMessage.SEVERITY_INFO,"", "Los alumnos se registraron correctamente");
		 			FacesContext.getCurrentInstance().addMessage(null, message); break;
		 	case 2: message = new FacesMessage(FacesMessage.SEVERITY_WARN,"", "Ya no quedan registros para guardar");
 					FacesContext.getCurrentInstance().addMessage(null, message); break;
		 	case 3: message = new FacesMessage(FacesMessage.SEVERITY_FATAL,"", "Ocurrió un error al guardar los datos");
					FacesContext.getCurrentInstance().addMessage(null, message); break;
		 }
	 }
	 ...
	 public String selectorDatosAlumnos(int modo) throws Exception{
		 ...
		 switch(modo){ 
			case 1: MODO_USUARIO = MODO_ADMIN;									
					...
					pagina = "/paginas/ModuloObservados/admin/cargar/cargarDatosAlumnosObs.xhtml"; break;
	...
	public void setDatosAlumnoExcelModelGrid(
			List<DatosAlumnoExcelModel> datosAlumnoExcelModelGrid) {
